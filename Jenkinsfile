...
checkout([$class: 'GitSCM', branches: [[name: '*/*']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/deaamoflih/demo-temp']]])
  ...
  stage('Code analysis') {
     ...
     sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName='testing' -Dsonar.projectKey='develop' -Dsonar.sources='src/' -Dsonar.language=java -Dsonar.java.binaries=* -Dsonar.branch=new " 
     }
  }
