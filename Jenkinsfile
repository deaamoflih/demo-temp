...
checkout([$class: 'GitSCM', branches: [[name: '*/*']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/deaamoflih/demo-temp']]])
  ...
  stage('Code analysis') {
     ...
     sh "${scannerHome}/bin/sonar-scanner -X -Dsonar.projectName='tesdsd' -Dsonar.projectKey='sdss' -Dsonar.sources='src/' -Dsonar.language=java -Dsonar.java.binaries=* -Dsonar.branch.name=new " 
     }
  }
