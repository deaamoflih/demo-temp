...
checkout([$class: 'GitSCM', branches: [[name: '*/*']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/deaamoflih/demo-temp']]])
  ...
  stage('Code analysis') {
     ...
     sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName='what' -Dsonar.projectKey='wd' -Dsonar.sources='src/' -Dsonar.language=java -Dsonar.java.binaries=* -Dsonar.branch.name=master " 
     }
  }
